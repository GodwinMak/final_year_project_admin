...
const Sidebar = () => {
  ...
  const filteredData = Data.filter(icon => {
    if (user.user.user.user.role === 'admin' && (icon.previllage.includes('admin') || icon.previllage.includes('all'))) {
      return true;
    } else if (user.user.user.user.role === 'user' && (icon.previllage.includes('user') || icon.previllage.includes('all'))) {
      return true;
    } else if (user.user.user.user.role === "root" &&(icon.previllage.includes("root") || icon.previllage.includes("all"))) {
      return true;
    } else {
      return false;
    }
    ...
    >
      ...
      <div className="h-full z-50 fixed drop-shadow-2xl md:drop-shadow flex">
        <div className="flex-col overflow-hidden md:overflow-auto justify-start items-start gap-4 flex bg-zinc-900 px-4 py-6 min-h-full">
          ...
          <div
            className="p-4 cursor-pointer rounded-lg flex-col text-white hover:text-white duration-300 justify-start items-center gap-2 flex"
        ...
        {insideSidebar &&
          (!specialInsideSidebar ? (
            ...
            (page.category === "real_time" && <RealTimeSidebar />) ||
            (page.category === "analysis" && <AnalysisSidebar />)
          ))}
      </div>
    </div>
  );
